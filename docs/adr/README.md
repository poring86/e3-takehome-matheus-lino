# ADR Index

This folder contains Architecture Decision Records (ADRs) for long-lived technical decisions.

## How to use

- Write ADRs for decisions that shape architecture, boundaries, data flow, or platform standards.
- Keep operational timelines and validations in `NOTES.md`.
- Link related `NOTES.md` entries to the ADR number.

## ADR List

- [0001 - Adopt modular monolith boundaries](./0001-adopt-modular-monolith-boundaries.md)
- [0002 - Standardize kebab-case filenames with fitness gate](./0002-standardize-kebab-case-filenames-with-fitness-gate.md)
- [0003 - Adopt React Query for dashboard server state](./0003-adopt-react-query-for-dashboard-server-state.md)
